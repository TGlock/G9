...
/* Router supports the following paths:
    ...
    Variable paths have known attributes:
        ...
        http method
        total # of segments
        a prefix of some length (can be zero)
        the segments that are fixed and those that are variable (dynamic)

            however, fixed and dynamic attributes cannot be determined during request processing ...
                'unless' a hint is used in dynamic segments to indicate as such e.g. a leading char
        ...
        The key to variable routes map is 'method' and '# of segments'
            variable route map entries are arrays
            for each entry
                if all fixed parts match, route is considered found.

        Collisions anticipated and allowed (thus use of array) but expect relatively few in actual use cases.
    ...
    get_routes = () => {
        ...
        for (let [key, entries] of this._vari) {
            for (let entry of entries) {
                paths.push(key + ' ' + entry.route.pattern)
            }
        }
        for (let [key, info] of this._wild) {
            paths.push( info.route.pattern)
        }
    ...
    */
    add_route = (path, methods, rte_obj_cbk) => {
        ...
        if (typeof rte_obj_cbk === 'function') {
            ...
            if (!route.execute) {
               console.log("warning route object does not have function named 'callback'")
            }
        }
        ...
        segments.forEach((segment, index) => {
            ...
            if (segment.startsWith(':') && (segment.includes(':', 1)) !== -1) {
    ...
    match = (path, method) => {
        ...
        let entries = this._vari.get(method + ' ' + this.get_segments(path))
        if (entries) {
            for (let entry of entries) {
        ...
        if ((method === 'HEAD') && (this._config.check_head)) {
            let r = this.match(path, 'GET')
            return (r) ? r : (this._not_found) ? {execute : this._not_found} : null
        } else {
            return (this._not_found) ? {execute : this._not_found} : null
        }
    }
