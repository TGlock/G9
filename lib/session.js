...
class Session {
    ...
    evict = () => {
        ...
        //console.log('sessions evicted:', n - map.size, 'time:', performance.now() - now)
    }
    ...
    expired(key) {
        return (this._map(key)?.last - performance.now()) > this._ttl
    }

    set(key, item) {
        this._map.set(key, { item: item, last: performance.now() })
        return item
    }

    add(item) {
        let key = this.key_create()
        this.set(key, { item: item, last: performance.now() })
        return key
    }
    ...
    get(key, touch = true) {
        ...
        if (item) {
            ...
            item.expired = ((now - item.last) > this._ttl)
        }
        return item?.item
    }
